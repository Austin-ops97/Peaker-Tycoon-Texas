...
- NP4-190-CD and NP6-905-CD artifact paths are pinned from the 2026-09-24 catalog extract. See [`Data/docs/ercot-adapter-pins.md`](Data/docs/ercot-adapter-pins.md). Twelve verified price zips are retained under `Data/archives/ercot/2026-09-24/` (`source_batch_count` 12, including six in-range NP6 zips in `batch2/`). That is not five-year coverage. 2021-02-15 is empty on the artifact API. NP4-180-ER and NP6-785-ER are absent from that catalog and still GATE. The OpenAPI file URL and credentials are still GATE. Internal pack use is owner-approved. Commercial App Store redistribution is not authorized.
