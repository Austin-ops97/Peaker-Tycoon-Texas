...
Design 04 (`Design/Peaker-Tycoon-Texas/04-iphone-nav-clarity-v1.md`) sets the stub hierarchy. Desk leads with Next up and `Review today’s plan`, then Today, then a muted retained NP4 sample strip. Chevron buttons cycle one row at a time: `HB_HOUSTON` and `HB_NORTH` on 2021-02-10, `HB_WEST` on 2021-04-15, then `HB_HOUSTON` on 2022-07-15, `HB_NORTH` on 2023-10-15, and `HB_WEST` on 2024-08-15. Fuel closed shows the four gas-day markers and no confirm button. Plant shows state, MW, and fuel flow as blanks. Settle’s only action explains settlement in plain language. There is no offer ticket.
...
- NP4-190-CD and NP6-905-CD artifact paths are pinned from the 2026-09-24 catalog extract. See [`Data/docs/ercot-adapter-pins.md`](Data/docs/ercot-adapter-pins.md). One hundred twelve verified price zips are retained under `Data/archives/ercot/2026-09-24/` (`source_batch_count` 112, including batch 7’s 24 zips). That is not five-year coverage. Live From/To is empty for 2023-10-15. Archive files now include a real-time interval pair for 2022-11-15, which was empty on the live fetch in batch 6. Earlier archive samples include 2021-04-15, 2022-07-15, 2023-01-15, and 2023-04-15. Earlier empty artifact days that this drop did not re-query include 2021-02-15, 2021-08-15, 2022-02-15, and 2023-08-15. NP4-180-ER and NP6-785-ER are absent from that catalog and still GATE. The OpenAPI file URL and credentials are still GATE. Internal pack use is owner-approved. Commercial App Store redistribution is not authorized.
