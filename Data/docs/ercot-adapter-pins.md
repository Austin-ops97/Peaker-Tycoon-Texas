...
`RetainedSettlementArchive` can open the verified price zips in batch2 through batch9 and parse the CSV member. That read path checks the `.sha256` sidecar first. It does not fetch, does not publish a complete pack, and does not fill proxy coverage. `Data/archives/ercot/2026-09-24/retained-sample-inventory.json` lists the CSV delivery dates from that read. The list is a sample inventory, not complete coverage.
