...
@Test func readingRetainedZipsDoesNotClaimCoverage() throws {
    ...
    #expect(manifest.sourceBatchCount == 112)
...
@Test func retainedSampleInventoryListsDeliveryDatesWithoutClaimingCoverage() throws {
    ...
    #expect(inventory.retainFolders == ["batch2", "batch3", "batch4", "batch5", "batch6", "batch7"])
    ...
    #expect(np4.zipCount == 54)
    #expect(np6.zipCount == 52)
    ...
    #expect(inventory.emptyLiveFromToGates.map(\.sourceLocalDate) == ["2023-10-15"])
    let october = try #require(inventory.emptyLiveFromToGates.first)
    #expect(october.retainedHourEndingsByProduct["NP4-190-CD"]?.count == 24)
    #expect(october.retainedHourEndingsByProduct["NP6-905-CD"]?.isEmpty == true)
...
@Test func retainedSamplePlayerSummaryStaysIncomplete() throws {
    ...
    #expect(summary.fileSentence == "54 day-ahead price files and 52 real-time price files are saved with this build.")
    #expect(summary.dateSentence.contains("54 day-ahead delivery dates"))
    #expect(summary.dateSentence.contains("46 real-time delivery dates"))
