...
@Test func retainedBatch7NP6ZipParsesNovemberFifteenthAfterSidecarCheck() throws {
    let url = retainRoot().appendingPathComponent(
        "batch7/np6-905-cd__inst1_20221115_876906627_SPPHLZNP6905_20221115_1145_csv.zip"
    )
    let read = try RetainedSettlementArchive.readZip(at: url, ingestedAt: retainedIngestedAt)
    #expect(read.product == .realtimeNP6905CD)
    #expect(read.result.errors.isEmpty)
    ...
    #expect(houston.pointType == "hub_spp")
    #expect(houston.valueDecimal == "35.69")
...
@Test func readingRetainedZipsDoesNotClaimCoverage() throws {
    ...
    #expect(manifest.sourceBatchCount == 229)
...
@Test func retainedSampleInventoryListsDeliveryDatesWithoutClaimingCoverage() throws {
    ...
    #expect(inventory.retainFolders == ["batch2", "batch3", "batch4", "batch5", "batch6", "batch7", "batch8", "batch9", "batch10"])
    ...
    #expect(np4.zipCount == 89)
    #expect(np6.zipCount == 104)
    ...
    #expect(inventory.emptyLiveFromToGates.map(\.sourceLocalDate) == ["2021-03-15"])
    let marchGate = try #require(inventory.emptyLiveFromToGates.first)
    #expect(marchGate.retainedHourEndingsByProduct["NP4-190-CD"]?.isEmpty == true)
    #expect(marchGate.retainedHourEndingsByProduct["NP6-905-CD"] == ["12:3", "12:4"])
    let februaryRealtime = try #require(np6.deliveryDates.first { $0.sourceLocalDate == "2022-02-15" })
    #expect(februaryRealtime.hourEndings == ["12:3", "12:4"])
    let octoberRealtime = try #require(np6.deliveryDates.first { $0.sourceLocalDate == "2023-10-15" })
    #expect(octoberRealtime.hourEndings.contains("12:3"))
    #expect(octoberRealtime.hourEndings.contains("12:4"))
    #expect(octoberRealtime.hourEndings.contains("7:3"))
    #expect(octoberRealtime.hourEndings.contains("17:3"))
    #expect(octoberRealtime.hourEndings.contains("24:3"))
    let octoberAhead = try #require(np4.deliveryDates.first { $0.sourceLocalDate == "2023-10-15" })
    #expect(octoberAhead.hourEndings.count == 24)
    let novemberRealtime = try #require(np6.deliveryDates.first { $0.sourceLocalDate == "2022-11-15" })
    #expect(novemberRealtime.hourEndings == ["12:3", "12:4"])
    let novemberAhead = try #require(np4.deliveryDates.first { $0.sourceLocalDate == "2022-11-15" })
    #expect(novemberAhead.hourEndings.count == 24)
...
@Test func retainedSamplePlayerSummaryStaysIncomplete() throws {
    ...
    #expect(summary.fileSentence == "89 day-ahead price files and 104 real-time price files are saved with this build.")
    #expect(summary.dateSentence.contains("89 day-ahead delivery dates"))
    #expect(summary.dateSentence.contains("75 real-time delivery dates"))
