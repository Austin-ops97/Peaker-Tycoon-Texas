...
@Test func retainedSampleInventoryListsDeliveryDatesWithoutClaimingCoverage() throws {
    ...
    #expect(inventory.retainFolders == ["batch2", "batch3", "batch4", "batch5", "batch6", "batch7", "batch8", "batch9"])
    ...
    #expect(np4.zipCount == 77)
    #expect(np6.zipCount == 88)
    ...
    #expect(inventory.emptyLiveFromToGates.map(\.sourceLocalDate) == ["2023-06-15"])
    let juneGate = try #require(inventory.emptyLiveFromToGates.first)
    #expect(juneGate.retainedHourEndingsByProduct["NP4-190-CD"]?.count == 24)
    #expect(juneGate.retainedHourEndingsByProduct["NP6-905-CD"]?.isEmpty == true)
...
@Test func retainedSamplePlayerSummaryStaysIncomplete() throws {
    ...
    #expect(summary.fileSentence == "77 day-ahead price files and 88 real-time price files are saved with this build.")
    #expect(summary.dateSentence.contains("77 day-ahead delivery dates"))
    #expect(summary.dateSentence.contains("63 real-time delivery dates"))
