...
@Test func retainedSampleInventoryListsDeliveryDatesWithoutClaimingCoverage() throws {
    ...
    #expect(inventory.retainFolders == ["batch2", "batch3", "batch4", "batch5", "batch6", "batch7", "batch8", "batch9", "batch10", "batch11"])
    ...
    #expect(np4.zipCount == 103)
    #expect(np6.zipCount == 120)
    ...
    #expect(inventory.emptyLiveFromToGates.isEmpty)
    let marchRealtime = try #require(np6.deliveryDates.first { $0.sourceLocalDate == "2021-03-15" })
    #expect(marchRealtime.hourEndings == ["12:3", "12:4"])
...
@Test func retainedSamplePlayerSummaryStaysIncomplete() throws {
    ...
    #expect(summary.fileSentence == "103 day-ahead price files and 120 real-time price files are saved with this build.")
    #expect(summary.dateSentence.contains("103 day-ahead delivery dates"))
    #expect(summary.dateSentence.contains("83 real-time delivery dates"))
