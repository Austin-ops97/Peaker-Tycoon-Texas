...
@Test func kernelAndAppSourcesDoNotReadTheWallClockOrImportUI() throws {
    ...
    #expect(fuel.contains("Nomination isn’t open right now, so this screen only shows the day’s timeline."))
    ...
    #expect(plant.contains("State, power output, and fuel flow stay blank until a reading is connected."))
