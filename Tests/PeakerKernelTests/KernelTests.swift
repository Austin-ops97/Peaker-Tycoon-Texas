...
@Test func plantUnitReadingUsesPlainEnglishAndAGamePhysicsSample() {
    let labels = Dictionary(uniqueKeysWithValues: PlantUnitReading.OperatingState.allCases.map { ($0.rawValue, $0.playerLabel) })
    #expect(labels["OFFLINE"] == "Offline")
    #expect(labels["READY"] == "Ready")
    #expect(labels["STARTING"] == "Starting")
    #expect(labels["PURGING"] == "Purging")
    #expect(labels["FIRING"] == "Firing")
    #expect(labels["ACCELERATING"] == "Accelerating")
    #expect(labels["SYNCHRONIZING"] == "Synchronizing")
    #expect(labels["ONLINE"] == "Online")
    #expect(labels["STOPPING"] == "Stopping")
    #expect(labels["COOLDOWN"] == "Cooldown")
    #expect(labels["TRIPPED"] == "Tripped")
    #expect(labels["MAINTENANCE"] == "Maintenance")
    #expect(labels.count == 12)
    #expect(PlantUnitReading.OperatingState.online.symbolName == "bolt.fill")
    #expect(PlantUnitReading.OperatingState.offline.symbolName == "powerplug")
    #expect(PlantUnitReading.OperatingState.tripped.symbolName == "exclamationmark.triangle")
    #expect(PlantUnitReading.OperatingState.ready.symbolName == nil)
    #expect(PlantUnitReading.playerFigure(Decimal(string: "48")!) == "48")
    #expect(PlantUnitReading.playerFigure(Decimal(string: "42.0")!) == "42")
    #expect(PlantUnitReading.playerFigure(Decimal(string: "42.5")!) == "42.5")
    #expect(PlantUnitReading.playerFigure(Decimal(string: "42.04")!) == "42")

    let empty = PlantUnitReading(state: nil, netMegawatts: nil, fuelFlowMMBtuPerHour: nil, evidence: .game)
    #expect(empty.hasRetainedField == false)
    let deratedOnly = PlantUnitReading(state: nil, derated: true, netMegawatts: nil, fuelFlowMMBtuPerHour: nil, evidence: .game)
    #expect(deratedOnly.hasRetainedField == false)
    let megawattsOnly = PlantUnitReading(state: nil, netMegawatts: Decimal(string: "12.5"), fuelFlowMMBtuPerHour: nil, evidence: .proxy)
    #expect(megawattsOnly.hasRetainedField)
    #expect(megawattsOnly.stateText == "—")
    #expect(megawattsOnly.megawattText == "12.5")
    #expect(megawattsOnly.fuelFlowText == "—")

    let sample = PlantUnitReading.gamePhysicsSample
    #expect(sample.evidence == .game)
    #expect(sample.evidence != .source)
    #expect(sample.state == .online)
    #expect(sample.derated == false)
    #expect(sample.hasRetainedField)
    #expect(sample.stateText == "Online")
    #expect(sample.megawattText == "48")
    #expect(sample.fuelFlowText == "480")
    #expect(sample.netMegawatts! * 10 == sample.fuelFlowMMBtuPerHour!)

    let derated = PlantUnitReading(state: .online, derated: true, netMegawatts: sample.netMegawatts, fuelFlowMMBtuPerHour: sample.fuelFlowMMBtuPerHour, evidence: .game)
    #expect(derated.stateText == "Online")
    #expect(derated.derated)
}
...
@Test func kernelAndAppSourcesDoNotReadTheWallClockOrImportUI() throws {
    ...
    #expect(!desk.contains("You’re on the trading desk"))
    #expect(!desk.contains("When offers open"))
    ...
    #expect(desk.contains("Hub price"))
    #expect(desk.contains("Not a repeated hour."))
    #expect(desk.contains("No publication time saved with this row."))
    ...
    #expect(fuel.contains("Nomination isn’t open right now. You can still see today’s timeline."))
    #expect(fuel.contains("Next, See gas day timeline."))
    #expect(fuel.contains("revealTimeline"))
    ...
    #expect(fuel.contains("Nomination is closed. These saved days have no publication time."))
    #expect(fuel.contains("Publication time isn’t saved with these rows."))
    #expect(fuel.contains("This is not a gas volume."))
    #expect(!fuel.contains("\"Confirm\""))
    #expect(!fuel.contains("\"Trade\""))
    let plant = try String(contentsOf: root.appendingPathComponent("App/PeakerTycoon/PlantPlaceholderView.swift"), encoding: .utf8)
    #expect(plant.contains("Read-only unit status. State, power, and fuel flow show here when live."))
    #expect(plant.contains("Next, View unit status."))
    #expect(plant.contains("revealStatus"))
    #expect(plant.contains("Not a live dispatch."))
    ...
    #expect(plant.contains("Unit status · read-only"))
    #expect(!plant.contains("Live unit readouts"))
    #expect(plant.contains("Game-physics sample."))
    #expect(plant.contains("GAME. Game-physics sample for"))
    #expect(plant.contains("MMBtu/h"))
    #expect(plant.contains("Derated"))
    #expect(plant.contains("navigationTitle(\"Unit status\")"))
    #expect(plant.contains("This screen shows the unit’s state, power output in megawatts, and fuel flow. It’s read-only — you can’t start or stop the plant from here."))
    #expect(!plant.contains("\"Confirm\""))
    #expect(!plant.contains("\"Trade\""))
    let settle = try String(contentsOf: root.appendingPathComponent("App/PeakerTycoon/SettlePlaceholderView.swift"), encoding: .utf8)
    #expect(settle.contains("Statements show up here after the day settles."))
    #expect(settle.contains("Next, How settlement works."))
    #expect(settle.contains("showSettlementExplainer"))
    #expect(settle.contains("Settle tab. Statements aren’t ready yet. This isn’t a live settle."))
    ...
    let waitingLine = try String(contentsOf: root.appendingPathComponent("App/PeakerTycoon/Chrome/EmptyLeadHeader.swift"), encoding: .utf8)
    #expect(waitingLine.contains("Button(action: action)"))
    #expect(waitingLine.contains("accessibilityHint"))
    ...
    #expect(shell.contains("accessibilityReduceMotion"))
    #expect(shell.contains("withoutMotionIfNeeded"))
    #expect(shell.contains("proxy.size.height * 0.45"))
    #expect(shell.contains("PlantPlaceholderView(reading: .gamePhysicsSample)"))
